  ...
  if (message.content === "<@!783211731279282186>") {
    ...
    if (dil == "TR_tr") {
      message.channel.send(
        `Prefixim: \`${pref}\`\nEğer yardım istiyorsan; https://discord.gg/WyWZRGY2Ry`
      );
    } else {
      message.channel.send(
        `My prefix is: \`${pref}\`\nIf you want to get help; https://discord.gg/WyWZRGY2Ry`
      );
    }
  } else {
    return;
...
client.on("message", async message => {
  ...
  if (message.content === "!") {
    ...
    if (dil == "TR_tr") {
      message.channel.send(
        "Galiba komut ismini unuttun, `!yardım`` yazarak gerekli komutlara erişebilirsiniz."
      );
    } else {
      message.channel.send(
